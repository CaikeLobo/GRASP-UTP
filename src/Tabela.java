public class Tabela {
 ...
 private static final int DIAS = 5; // Segunda a Sexta
    ...
    public boolean inserirAula(int dia, int horario, Materia materia) {
        // Validação básica: dia e horário válidos
        ...
        if (!Grade[dia][horario].equals("Livre")) {
            System.out.println("Erro: Horário já ocupado por: " + Grade[dia][horario]);
            return false;
        }
        
        Grade[dia][horario] = materia.getNomeMat() + " (" + materia.getProfessor().getNome() + ")";
        System.out.println("Aula de " + materia.getNomeMat() + " inserida com sucesso.");
        return true;
    }
       ...
       // System.out.println("\n--- Grade de Horários do " + this.nomeCurso + " ---");
        ...
        for (int i = 0; i < HORARIOS; i++) {
            ...
            for (int j = 0; j < DIAS; j++) {
                System.out.printf("%-25s", Grade[j][i]);
            }
            System.out.println();
        }
        System.out.println("----------------------------------------------");
